...
import { CardOperationSweepService } from './card-operation-sweep.service';

/**
 * A dynamically-registered interval, for the reason the sweeps beside it are:
 * its cadence comes from configuration, so the cadence being read, the timer
 * being registered and the timer being torn down are exactly what a regression
 * would silently drop.
 */
describe('CardOperationSweepService', () => {
  let service: CardOperationSweepService;
  let resolveOperations: { execute: jest.Mock };
  ...
  beforeEach(async () => {
    ...
    resolveOperations = { execute: jest.fn().mockResolvedValue(undefined) };
    schedulerRegistry = {
      addInterval: jest.fn(),
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ResolveCardOperationsUseCase, useValue: resolveOperations },
        { provide: SchedulerRegistry, useValue: schedulerRegistry },
        { provide: ConfigService, useValue: configService },
      ],
    }).compile();

    service = module.get(CardOperationSweepService);
  });
  ...
  describe('registration', () => {
    ...
    it('runs at its own configured cadence, not a shared one', () => {
      ...
      // Its own variable is the point: one issuer works these by hand over
      // hours, so a shared number is wrong for this pass either way.
    ...
    it('does not sweep before the first interval elapses', () => {
      ...
      expect(resolveOperations.execute).not.toHaveBeenCalled();
    });
    ...
    it('sweeps once the interval elapses', () => {
      ...
      expect(resolveOperations.execute).toHaveBeenCalledTimes(1);
    });
  ...
  describe('the pass', () => {
    it('swallows a failed pass rather than rejecting into the timer', async () => {
      ...
      resolveOperations.execute.mockRejectedValueOnce(
  ...
  describe('one pass at a time', () => {
    it('skips a tick while the previous pass is still running', async () => {
      ...
      // Two overlapping passes would both read the same operation before
      // either had written its outcome, and both would move the card.
      let release = (): void => {};
      resolveOperations.execute.mockImplementationOnce(
        () =>
    ...
    it('accepts the next pass once the previous one finishes', async () => {
      ...
      expect(resolveOperations.execute).toHaveBeenCalledTimes(2);
    });
